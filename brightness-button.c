...
#define XFPM_DISPLAY_BRIGHTNESS_INVALID_ICON	"dialog-warning-symbolic"
...
G_DEFINE_TYPE_WITH_PRIVATE (BrightnessButton, brightness_button, GTK_TYPE_BUTTON)
...
minus_clicked (GtkWidget *widget, BrightnessButton *button)
{
    ...
    if ( level >= 500 )
	gtk_range_set_value (GTK_RANGE (button->priv->range), level - 500);
}
...
{
    button->priv = brightness_button_get_instance_private (button);
